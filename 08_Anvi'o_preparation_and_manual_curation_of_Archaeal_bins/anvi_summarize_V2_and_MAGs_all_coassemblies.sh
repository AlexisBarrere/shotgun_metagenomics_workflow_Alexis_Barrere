...
anvi_summarize_V2_and_MAGs_all_coassemblies() {
  ...
  COLLECTION_NAME_ARCHAEA=metawrap_bins_${GROUP}_coassembly_Archaea_V2

  COLLECTION_NAME_MAGS=${GROUP}_MAGs

  # === Outputs ===
  OUT_DIR_1=$ROOT/anvi_summarize/${GROUP}_Archaea_V2_summarize

  OUT_DIR_2=$ROOT/anvi_summarize/${GROUP}_MAGs_summarize
...
export -f anvi_summarize_V2_and_MAGs_all_coassemblies

# === Run the function ===
tail -n +2 ~/Stage_Copenhague/visualisation_anvio/coassembly_groups.tsv | \
cut -f1 | \
parallel -j 2 --bar 'anvi_summarize_V2_and_MAGs_all_coassemblies {}'
