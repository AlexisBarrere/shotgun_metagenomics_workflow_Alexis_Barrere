...
create_Verrucomicrobiota_collections() {
  ...
  INITIAL_COLLECTION=$COLLECTION_DIR/metawrap_bins_${GROUP}_coassembly.txt
  INITIAL_INFO=$COLLECTION_DIR/metawrap_bins_${GROUP}_coassembly-info.txt

  COLLECTION_NAME_VERRUCOMICROBIOTA=metawrap_bins_${GROUP}_coassembly_Verrucomicrobiota
  ...
  SUMMARIZE_VERRUCOMICROBIOTA=$ANVI_SUMMARIZE_DIR/${GROUP}_Verrucomicrobiota_summarize
  ...
  echo "Done. Verrucomicrobiota-specific collection and info files created for the $GROUP group :"
...
cut -f1 | \
parallel -j 2 --bar 'create_Verrucomicrobiota_collections {}'
