...
run_binning() {
  ...
  # === Directories ===
  OUT_DIR=~/Stage_Copenhague/binning/${GROUP}_concoct
  mkdir -p $OUT_DIR
...
# Read the seventh line (the T7_N1P2K2 group) and pass the columns to the function
read GROUP S1 S2 S3 S4 < <(sed -n '7p' ~/Stage_Copenhague/assembly/coassembly_groups.tsv)
